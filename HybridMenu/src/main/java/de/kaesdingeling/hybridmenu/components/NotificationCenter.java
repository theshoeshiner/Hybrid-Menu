...
public class NotificationCenter extends VerticalLayout {
	...
	public NotificationCenter() {
		...
		addFooter(HMButton.get().withIcon(VaadinIcon.ANGLE_RIGHT.create()).withStyleName(Styles.buttonIconOnly).withClickListener(e -> close()));
	}
	...
	public NotificationCenter updateToolTip() {
		if (notiButton != null) {
			...
			if (unreaded > 0) {
				notiButton.setIcon(VaadinSession.getCurrent().getAttribute(MenuConfig.class).getNotificationButtonIcon().create());
			} else {
				notiButton.setIcon(VaadinSession.getCurrent().getAttribute(MenuConfig.class).getNotificationButtonEmptyIcon().create());
			}
		}
		return this;
	}
